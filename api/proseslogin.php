...
if (isset($_POST['login'])) {
    ...
    // ===== DEBUG SEMENTARA =====
    // Hapus bagian ini setelah login berhasil!
    if (!$data) {
        die("DEBUG: User '$username' tidak ditemukan di database.");
    }
    $passwordHash = $data['password'];
    $verifyResult = password_verify($pass, $passwordHash);
    if (!$verifyResult) {
        die("DEBUG: Password salah.<br>
             Input: " . htmlspecialchars($pass) . "<br>
             Hash di DB: " . htmlspecialchars($passwordHash) . "<br>
             Panjang hash: " . strlen($passwordHash));
    }
    // ===== END DEBUG =====
